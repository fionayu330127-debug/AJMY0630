...
async function renderDataCenter() {
  ...
  try {
    ...
    const detail = d.inviteRows.map(r=>`<tr><td>${esc(r.name)}</td><td>${r.totalCreators}</td><td>${r.targetedInvites}</td><td>${r.openInvites}</td><td>${r.sampleCooperations}</td></tr>`).join('') || '<tr><td colspan="5">暂无数据</td></tr>';
